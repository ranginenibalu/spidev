...
static void spi() // Function to transfer via SPI
{
	...
	struct spi_ioc_transfer tr = {
		...
		while(1)
		{
			...
			{
				strcpy(s1.rx , rx );
				strcpy(s1.file, "log_spi.dat");  
				pthread_create( &ts, NULL, convert ,(void *) &s1);
...

{
	...
	while(1)
	{
		...
		else
		{
			...
			{
				strcpy(u1.rx , rx );
				strcpy(u1.file, "log_uart.dat");
				pthread_create(&tu, NULL, convert ,(void *)&u1); // create a thread running function2
			}
